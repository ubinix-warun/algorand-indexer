...
func DaemonCmd() *cobra.Command {
	...
	daemonCmd := &cobra.Command{
		Use:   "daemon",
		Short: "run webhook daemon",
		Long:  "run webhook daemon. Serve api on HTTP.",
		//Args:
...
func loadIndexerParamConfig(cfg *daemonConfig) error {
	...
	if paramConfigFound {
		...
		logger.Infof("Auto-loading parameter configuration file: %s", cfg.suppliedAPIConfigFile)
	}
	return err
}
...
func runDaemon(daemonConfig *daemonConfig) error {
	...
	if bot != nil {
		wg.Add(1)
		go runBlockProcesser(ctx, daemonConfig, &wg, db, availableCh, bot, opts)
	} else {
		logger.Info("No block processer configured.")
	}
...
func runBlockProcesser(ctx context.Context, cfg *daemonConfig, wg *sync.WaitGroup, db idb.IndexerDb, dbAvailable chan struct{}, bot fetcher.Fetcher, opts idb.IndexerDbOptions) {
	// Need to redefine exitHandler() for every go-routine
	defer exitHandler()
	defer wg.Done()
